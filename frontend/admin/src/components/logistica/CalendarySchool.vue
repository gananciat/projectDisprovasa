                ...
                <b-jumbotron :header="'Historial de la Calendarización del año '+anio">
                    <div class="timeline">
                        <template v-for="item in Object.values(items)">
                          ...
                          <template v-for="(iitem, iindex) in item">
                            <div class="time-label" v-bind:key="'1.'+iitem.id">
                                <span :class="color(iitem.date)">{{ iitem.date | moment('dddd DD MMMM YYYY') }}</span>
                            </div>
                            <div  v-bind:key="'2.'+iitem.id">
                                <i class="fa fa-calendar"></i>
                                <div class="timeline-item">
                                    <span class="time">
                                        <button type="button" class="btn btn-danger btn-sm" v-if="puede_eliminar(iitem.date)" v-b-tooltip.hover title="eliminar" @click="destroy(item)">
                                            <i class="fa fa-trash">
                                            </i>
                                        </button>                                        
                                    </span>
                                    <h3 class="timeline-header">#{{ iindex+1 }} <b-badge variant="primary">{{ iitem.title }}</b-badge></h3>
                                    <div class="timeline-body">
                                        {{ iitem.school.name }}
                                        <br>
                                        ESTÁ ACTIVIDAD LA REGISTRO LA PERSONA {{ concat_name(iitem.person.name_one,iitem.person.name_two,iitem.person.last_name_one,iitem.person.last_name_two) }}
                                    </div>
                                </div>
                            </div>
                          </template>
          ...
          if (result.value) { 
              ...
              self.$store.state.services.calendaryschoolService
                ...
                .then(r => {
                  self.loading = false
                  if( self.interceptar_error(r) == 0) return
                  self.$toastr.success(self.r.data.data.length+' escuelas fueron agregadas al calendario.', 'Mensaje') 
                  self.getAll()
                })
                .catch(r => {});
          }
